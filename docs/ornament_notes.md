...
- **PENDING BAKE (round 8; lead's merge correction 2026-09-09, docs/reviews/orn_r8_review.md)**: `attic_panel` was baked on main by the
  lead after r7 (037dc59) and is NOT pending; the merge kept main's ornament.blend. Only `capital_rotunda` (rebuilt `--no-bake` in r8) is
  pending: `orn_build.py --only capital_rotunda` (with bake) runs in the lead's next GPU window. `--bake-pending` prints
