# QA round 23 — Phase 8 closing round: 8d belt r2 (real crowns) + parity on the Phase 8 Cycles references; 8a/8e/fix-round regression, on the live site (brief from the lead,
...
ROUND 23 DELTAS (read these first; they override the method above where they conflict): before = the `gate10` set (QA 22); read docs/qa_round_22.md in full (the 8d blocker:
faceted belt at 1/2/5 and in the reflection; cam05 crossings 10.80; the MAE-vs-Cycles rise at backdrop boxes), docs/decisions.md "QA 22", "8a-4 decision", the belt r2
entries, docs/briefs/phase8d_belt_r2_report.md, docs/briefs/phase8_cycles_refs.md and the ENV builder's report (the Phase 8 Cycles references in renders/qa_comparisons/
cycles_p8/ — PARITY at every backdrop/belt box is measured against THESE; the architecture boxes against both sets, which must agree within 1 %). (1) 8d: the belt at 100 %
at 1, 2, 5 and in the hero reflection — tree crowns with sky-gapped silhouettes and trunks, no shards, no hedge of identical silhouettes, no floating crown, tops below the
hall roofline; cam06 city as before (R1 only). Verdict 8d CLOSED / NOT on the tiles. (2) far crowns: cam05 crossings must return within 0.3 of QA 21's 11.67 or be
explained by the belt backing. (3) 8a: closed with residual (cam03 lighting carry) — regression only; leaf share reported. (4) 8e: the 2.5 m walk-up frame is in this capture
(`<tag>_walkup*.png`) — check the close cards (coverage, no fattening). (5) fix round + payload + perf + resident + name sweep as above; payload expected to move only by
the belt's rows and the re-baked backdrop maps. Scores and deltas vs QA 22 and QA 21. Verdict: PHASE 8 ART ITEMS CLOSED (8a with residual, 8b, 8c, 8d, 8e) or the named
remaining blocker with its owner. Write docs/qa_round_23.md, renders/web/<tag>_gate.png, append docs/quality_checklist.md; commit only those + scripts/qa_r23_*.py.
