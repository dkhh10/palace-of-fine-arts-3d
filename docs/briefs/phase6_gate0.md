...
4. `export/bake_lightmap.py`: Cycles Diffuse direct+indirect, colour off, `light_presets.apply_final_cycles` first (the
...
5. `export/bake_lut.py`: (a) 33^3 LUT, scene-linear -> display through the scene's view settings (AgX High Contrast,
   exposure -2.833) as a `.cube` or Hald PNG; (b) two 4096x2048 equirect EXRs of the world: camera branch and glossy
...
7. Reference frame: Cycles 1280x720, 64 spp, of the SLICE ONLY (column x16 + ground + world) from `CAM_qa_01_lagoon_hero`,
...
3. Acceptance: `renders/web/gate0_pair.png` = Cycles frame | viewer frame | 50 % blend, plus a 100 % crop of the column
   (both frames). Report the column's pixel bounding box in both (must agree within 1 % of frame height), the mean luminance
