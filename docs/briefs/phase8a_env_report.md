...
**The change** (`scripts/env_build.py`). Cards are emitted as **clumps** (`_leaf_clump`: 4–9 cards, four leaf scales with mean square 1.0, shared dominant azimuth ± 0.85 rad, tilt spread ± 0.55) instead of an even scatter of one-size quads; blade clumps come in **tufts** of 4–8 sharing an origin and lean. Clumping now runs at **all three LODs**. The round-02 "k× wider, k²× fewer" rule is relaxed at LOD1 (`card 2.20 / cover 0.95 / blade 0.33` → **`1.65 / 1.00 / 0.75`**: leaf card 18.7 → **14.0 cm**, 1.87× the cards; reed blade 7.8 → **5.2 cm**, 2.3× the blades) and at LOD2 (`4.50 / 0.85 / 0.12` → **`3.30 / 0.85 / 0.24`**: leaf card 38.3 → **28.1 cm**, 1.86× the cards; reed blade 13.0 → **9.2 cm**, 2.0× the blades). LOD0's card count and card size are untouched, so its triangle total is unchanged and only the distribution moves. No material, name, placement or LOD-assignment change (1 379 placements, 25 sources × 3 LODs, `MAT_shrub{,_light,_dry}` / `MAT_reeds` untouched).
