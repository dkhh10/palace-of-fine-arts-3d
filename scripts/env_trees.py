...
SPECIES = {
    ...
    "cypress": dict(
        levels=3, length=(1.0, 0.55, 0.42, 0.0), lengthV=(0.0, 0.15, 0.2, 0.0), branches=(0, 26, 16, 0),
        curveRes=(8, 6, 4, 1), curve=(0.0, -15.0, -10.0, 0.0), curveV=(30.0, 70.0, 80.0, 0.0),
        shape="8", customShape=(0.75, 1.0, 0.45, 0.35), branchDist=1.2, baseSize=0.22, baseSize_s=0.3,
        ...
        rotateV=(20.0, 30.0, 40.0, 0.0), attractUp=(0.0, 0.7, 0.5, 0.0), segSplits=(0.0, 0.25, 0.1, 0.0),
        splitAngle=(0.0, 25.0, 20.0, 0.0), splitAngleV=(0.0, 8.0, 8.0, 0.0), splitByLen=True, splitHeight=0.3,
        scale=20.0, scaleV=2.0, leaves=14, leafScale=0.95, leafScaleX=1.0, leafScaleV=0.3, bend=0.35, leafangle=5.0,
        leafDownAngle=35.0, leafDownAngleV=20.0, leafRotate=137.5, leafRotateV=30.0,
        bark="MAT_bark_cypress", leaf="MAT_leaf_cypress", height=(15, 25)),
    # tall straight trunk, high open crown, hanging foliage
    "eucalyptus": dict(
        levels=3, length=(1.0, 0.38, 0.45, 0.0), lengthV=(0.0, 0.2, 0.25, 0.0), branches=(0, 22, 12, 0),
        curveRes=(10, 6, 4, 1), curve=(0.0, 10.0, -25.0, 0.0), curveV=(25.0, 60.0, 90.0, 0.0),
        shape="8", customShape=(0.35, 1.0, 0.55, 0.55), branchDist=1.4, baseSize=0.4, baseSize_s=0.35,
        ratio=0.016, ratioPower=1.3, scale0=1.0, scaleV0=0.1, rootFlare=1.15,
        downAngle=(90.0, 45.0, 55.0, 45.0), downAngleV=(0.0, -30.0, 20.0, 10.0), rotate=(137.5, 137.5, 137.5, 137.5),
        rotateV=(20.0, 20.0, 40.0, 0.0), attractUp=(0.3, 0.5, -0.9, 0.0), segSplits=(0.15, 0.3, 0.0, 0.0),
        splitAngle=(20.0, 25.0, 0.0, 0.0), splitAngleV=(5.0, 8.0, 0.0, 0.0), splitByLen=True, splitHeight=0.35, baseSplits=1,
        scale=30.0, scaleV=3.0, leaves=12, leafScale=1.25, leafScaleX=0.38, leafScaleV=0.3, bend=0.2, leafangle=-40.0,
    ...
    "pine": dict(
        levels=3, length=(1.0, 0.5, 0.4, 0.0), lengthV=(0.0, 0.2, 0.2, 0.0), branches=(0, 28, 14, 0),
        curveRes=(8, 5, 3, 1), curve=(0.0, -10.0, 0.0, 0.0), curveV=(35.0, 60.0, 60.0, 0.0),
        shape="8", customShape=(0.55, 1.0, 0.5, 0.45), branchDist=1.1, baseSize=0.3, baseSize_s=0.3,
        ratio=0.02, ratioPower=1.25, scale0=1.0, scaleV0=0.1, rootFlare=1.2,
        downAngle=(90.0, 60.0, 45.0, 45.0), downAngleV=(0.0, -30.0, 10.0, 10.0), rotate=(137.5, 137.5, 137.5, 137.5),
        ...
        scale=21.0, scaleV=2.0, leaves=16, leafScale=0.85, leafScaleX=0.9, leafScaleV=0.3, bend=0.3, leafangle=0.0,
        leafDownAngle=40.0, leafDownAngleV=25.0, leafRotate=137.5, leafRotateV=40.0,
        bark="MAT_bark_cypress", leaf="MAT_leaf_cypress", height=(15, 24)),
    # weeping willow at the water's edge
    "willow": dict(
        levels=3, length=(0.75, 0.5, 1.4, 0.0), lengthV=(0.0, 0.1, 0.1, 0.0), branches=(0, 24, 10, 0),
        ...
        scale=11.0, scaleV=1.5, leaves=30, leafScale=0.7, leafScaleX=0.18, leafScaleV=0.3, bend=0.0, leafangle=-60.0,
    ...
    "redwood": dict(
        ...
        scale=30.0, scaleV=3.0, leaves=10, leafScale=0.8, leafScaleX=0.9, leafScaleV=0.3, bend=0.2, leafangle=0.0,
        leafDownAngle=25.0, leafDownAngleV=10.0, leafRotate=137.5, leafRotateV=20.0,
...
SEEDS = {"cypress": (3, 17, 41), "eucalyptus": (5, 23, 61), "pine": (7, 29), "willow": (11, 37), "redwood": (13, 43), "broadleaf": (19, 53)}

# LOD overrides: geometry resolution and leaf density (skeleton stays identical for the same seed)
LOD_OVERRIDES = {
    0: dict(bevelRes=2, resU=3),
    1: dict(bevelRes=1, resU=2, leaf_factor=0.55, leaf_scale=1.25),
...
PLAN = [
    # A. hero right (north-east of the rotunda): the dense dark cluster, ref 169 / user image right of the rotunda
    ("redwood", -27.0, 9.0, 16.0, "A young redwood in front of the north arch (ref 070)"),
    ("pine", -31.0, 14.0, 21.0, "A cluster core"),
    ("cypress", -38.0, 3.0, 24.0, "A tallest dark mass right of the dome"),
    ("pine", -45.0, 13.0, 23.0, "A"),
    ("cypress", -36.0, 19.0, 19.0, "A"),
    ("pine", -52.0, 6.0, 22.0, "A back"),
    ("cypress", -58.0, 12.0, 26.0, "A tall column at the north pylon (user image x~1020)"),
    ("willow", -31.0, 27.0, 9.0, "A pale weeping willow at the water in front of the cluster (ref 169)"),
    ("broadleaf", -44.0, 24.0, 12.0, "A shore broadleaf"),
    # B. north shore beyond the north pylon
    ("eucalyptus", -66.0, 18.0, 28.0, "B"),
    ("cypress", -72.0, 8.0, 22.0, "B"),
    ("eucalyptus", -80.0, 24.0, 30.0, "B"),
    ("willow", -70.0, 35.0, 10.0, "B willow at the water (ref 144/145)"),
    ("pine", -90.0, 14.0, 20.0, "B"),
    ("eucalyptus", -100.0, 30.0, 30.0, "B big eucalyptus behind the willows (ref 171)"),
    ("willow", -95.0, 45.0, 9.0, "B"),
    # C. hero left (south-east of the rotunda)
    ("cypress", 34.0, 7.0, 25.0, "C cypress column left of the rotunda (user image x~290)"),
    ("cypress", 39.0, 15.0, 23.0, "C second cypress (user image x~330)"),
    ("broadleaf", 26.0, 15.0, 11.0, "C small tree touching the rotunda's left edge"),
    ("eucalyptus", 48.0, -18.0, 30.0, "C broad eucalyptus behind the south wing"),
    ("pine", 20.0, -12.0, 18.0, "C satellite crown (19,-13)"),
    ("cypress", 28.0, -3.0, 22.0, "C satellite crown (26,-3)"),
    # D. south shore near the south wing and pylon
    ("cypress", 62.0, 24.0, 20.0, "D dense cypress behind the south pylon (ref 169 far left)"),
    ("eucalyptus", 72.0, 8.0, 28.0, "D"),
    ("pine", 56.0, 40.0, 18.0, "D"),
    ("willow", 66.0, 48.0, 9.0, "D"),
    ("eucalyptus", 85.0, 20.0, 30.0, "D"),
    ("cypress", 95.0, 42.0, 22.0, "D"),
    # F. east and south shore (behind the hero camera): blue-gum rows and NE-corner cypresses (refs 141, 169, 105)
    ...
    ("eucalyptus", -58.0, 116.0, 28.0, "F"),
    ("cypress", -110.0, 133.0, 22.0, "F"),
    ("eucalyptus", -158.0, 131.0, 30.0, "F"),
    ("cypress", -120.0, 100.0, 24.0, "F NE-corner Monterey cypress (DPR, Harbor View Inn era)"),
    ("cypress", -140.0, 82.0, 22.0, "F"),
    ("eucalyptus", -100.0, 118.0, 30.0, "F"),
    ("broadleaf", 20.0, 128.0, 12.0, "F east lawn"),
    # G. wooded islet (lagoon1)
    ("willow", -95.0, 60.0, 9.0, "G islet willow"),
    ("broadleaf", -105.0, 68.0, 12.0, "G"),
    ("eucalyptus", -112.0, 62.0, 24.0, "G"),
    ("willow", -85.0, 70.0, 8.0, "G"),
    ("broadleaf", -118.0, 74.0, 10.0, "G"),
    # H. backdrop trees (Presidio side, Marina, Palace Drive) from z18 crowns - LOD1 capped
...
def build_all(SUB, terrain_height, lagoon_field, islet_fields, quick=False, colonnade_polys=None, hall_poly=None):
    ...
    for i, (sp, x, y, h, note) in enumerate(plan):
        ...
        # ground: trees stand on land; if a coordinate falls in the water (OSM shore vs photo), nudge to the shore
        d = lagoon_field.signed(x, y)
        if d < 1.0 and not any(f.signed(x, y) < 0 for f in islet_fields):
            for _ in range(20):
                x += (0 - x) * 0.05
                y += (0 - y) * 0.05
...
if __name__ == "__main__":
    ...
    ground.from_pydata([(-50, -50, 0), (400, -50, 0), (400, 50, 0), (-50, 50, 0)], [], [(0, 1, 2, 3)])
    ...
    for (sp, seed), lods in lib.items():
        for lod in (0, 1, 2):
            ...
            o.location = (x, -lod * 40.0, 0.0)
            coll.objects.link(o)
        x += SPECIES[sp]["height"][1] * 1.3
    # camera + light
    cam = bpy.data.cameras.new("cam")
    cam.lens = 35
    co = bpy.data.objects.new("cam", cam)
    coll.objects.link(co)
    co.location = (x / 2, 150.0, 22.0)
    co.rotation_euler = common.lookat_rotation(co.location, (x / 2, 0.0, 14.0))
    scene.camera = co
    ...
    out = common.RENDERS / "previews" / "environment" / f"{common.timestamp()}_tree_lineup.png"
    out.parent.mkdir(parents=True, exist_ok=True)
    scene.render.filepath = str(out)
    bpy.ops.render.render(write_still=True)
    print("[env_trees] lineup:", out)
    for lod in (0, 1, 2):
        co.location = (x / 2, 150.0 - lod * 40.0, 22.0)
        co.rotation_euler = common.lookat_rotation(co.location, (x / 2, -lod * 40.0, 14.0))
        scene.render.filepath = str(out).replace("lineup", f"lineup_LOD{lod}")
        bpy.ops.render.render(write_still=True)
