...
Row B: Cycles 4K hero BEFORE (phase5_v2) vs AFTER (renders/final/hero_cam01_3840x2160_128spp.png).
  ...
  KILLED (obsolete)"). Until AAFTER's mtime/hash differs from BEFORE, the script uses BEFORE as a
  stand-in for AFTER and prints a warning. Re-run this script once the real render lands.
Row C: reference photo (ref 169, letterboxed to the 16:9 hero framing) vs Cycles AFTER.

Outputs:
  renders/qa_comparisons/phase8_before_after_960.jpg   (committed)
  renders/qa_comparisons/phase8_tiles/*.png + boxes.json  (gitignored, full resolution)
  renders/qa_comparisons/phase8_stations/phase8_camNN_960.jpg  (--stations; committed)
"""
import json
import sys
...
# This script may run from a worktree (e.g. .claude/worktrees/phase8-sheet), which does not
...
TILES_DIR = QA / "phase8_tiles"
STATIONS_DIR = QA / "phase8_stations"

CYCLES_BEFORE = FINAL / "phase5_v2/hero_cam01_3840x2160_128spp.png"
...
def build_hero_sheet():
    ...
    if cycles_after_is_standin:
        print(f"WARNING: {cycles_after_path.relative_to(ROOT)} is not a post-Phase-8 render yet "
              f"(identical to BEFORE, or missing). Using BEFORE as a stand-in for AFTER. "
              f"Re-run this script once the Phase 8 4K hero lands.")
    ...
    ca_label_line2 = "post-Phase 8 4K hero" if not cycles_after_is_standin else \
        "STAND-IN = BEFORE FILE (Phase 8 4K hero not rendered yet)"
    cb = _panel(_im(cycles_before_path), col_w,
                [(cb_label, True), ("Phase 5 v2, pre-Phase 8", False)])
    ...
    header = _label_bar(row1.width, [("Phase 8 before / after - Palace of Fine Arts lagoon hero (cam01)", True)],
                         height=30, bg=(0, 0, 0))
    sheet = _stack_rows([header, row1, row2, row3])
    out_path = QA / "phase8_before_after_960.jpg"
...
def build_stations():
    ...
    for st in STATIONS:
        ...
        out_path = STATIONS_DIR / f"phase8_{st}_960.jpg"
        sheet.convert("RGB").save(out_path, quality=92)
        print(f"wrote {out_path.relative_to(ROOT)}  {sheet.size}")
