...
if "ORN" in linked and "ARCH" in linked:
    ...
    for idx_all, sk in enumerate(sorted(sockets, key=lambda o: o.name)):
        ...
        if t == "frieze_run":                 # ORN r5: rotunda rinceau only (fronts > 4 m, returns); colonnade runs stay empty
...
if "LIGHT" in linked:
    try:
        ...
        light_presets.apply_viewport_eevee(scene)   # review fix: saved Eevee state carries the Eevee vault override
