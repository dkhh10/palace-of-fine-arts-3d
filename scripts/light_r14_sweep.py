...
of round 13). Round-13 header follows.

New keys:
  * `tap` / `thp` -- the EXPONENTS on the anti-sun and horizon weights of the diffuse tint (QA-06-2). Round 12
    shipped both weights at 1.0, their maximum AMOUNT; the exponent is the SHARPNESS, which is what separates a
    shaded wall from anything that faces up (docs/lighting_notes.md 24.1).
...
for c in CASES:
    if any(abs(c[k] - DEFAULTS[k]) > 1e-9 for k in SKY_KEYS) and c["sky"] not in _calib:
        ...
        print(f"[r14] calibrate sky {c['sky']:g}: lamp {m['lamp_energy']:.2f} W/m2 "
...
print(f"[r14] master {MASTER}: {_N_OBJ} objects, exposure {EXPOSURE0:.4f}, look {scene.view_settings.look!r}, "
...
def apply_case(c):
    ...
    if any(abs(c[k] - DEFAULTS[k]) > 1e-9 for k in SKY_KEYS):
        ...
        old_w = bpy.data.worlds.get(f"R14_{case_tag(c)}")
        if old_w:
            bpy.data.worlds.remove(old_w)          # r12 review carry 9: do not leak one world per case
        w = cal.make_sky_world(f"R14_{case_tag(c)}", AZ, EL, lb.SKY, sun_disc=False, strength=c["sky"],
    ...
    if c["bake"] > 2.5:
        ...
        print("[r14] light-probe caches FREED: the frame is lit by the world SH and the lamps only", flush=True)
    elif c["bake"] > 0.5:
        lighting = c["bake"] < 1.5
        t = time.time()
        probes.bake(scene, lighting_world=lighting)
        print(f"[r14] re-baked with the {'LIGHTING (r13)' if lighting else 'SCENE/CAMERA (r12)'} world "
              f"in {time.time()-t:.0f}s", flush=True)
    print(f"[r14] case {case_tag(c)}: bake {c['bake']:g} comp {c['comp']:g} mist start {c['ms']:g} depth {c['md']:g} "
          f"haze cap {c['hz']:g} k {c['hk']:g} db {c['db']:g} tint {c['tr']:g},{c['tg']:g},{c['tb']:g} "
          f"ta {c['ta']:g}^{c['tap']:g} th {c['th']:g}^{c['thp']:g} f {c['f']:g} v {c['v']:g} "
...
def shoot(cam_id, tag):
    ...
    if eng == "c":
        ...
        print(f"[r14] eevee: fast_gi {e.use_fast_gi} dist {getattr(e, 'fast_gi_distance', 0):.1f} rays "
    ...
    print(f"[r14] -> {fp.name} ({dt:.1f}s, {res[0]}x{res[1]})", flush=True)
...
print(f"[r14] camera times: " + "  ".join(f"{k} {v:.1f}s" for k, v in TIMES) +
      f"   TOTAL {sum(v for _, v in TIMES):.1f}s over {len(TIMES)} frames", flush=True)
print("[r14] done", flush=True)
