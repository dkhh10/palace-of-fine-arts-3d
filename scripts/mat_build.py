...
# =============================================================================== Phase 9 / 8d R3: the tiled half
# R1 fixed the LOW-frequency band the 1K backdrop atlas can hold (0.79 texels/m on facades, 0.35 on forest).  R3
# is the high-frequency half and it cannot come from the atlas at any resolution: 2048 buys 1.58 texels/m and the
# hall's east wall is 7.0 screen px/m at the hero, so a 4 m bay is 28 px and a baked texel is 9 px.
#
# The answer is a REPEAT-sampled tile on the UV0 that `scripts/env_p9_uv0.py` lays in TILE UNITS (u along the
# street / v = height on walls, planar world XY on roofs and canopy tops).  The images are generated by
# `scripts/env_p9_tiles.py` as mean-0.5 GAIN maps, so
#       albedo <- albedo * (1 + (tex - 0.5) * 2 * strength)
# has a mean gain of exactly 1.0 and R1's luma / saturation numbers survive.  `backdrop_atmosphere` still runs
...
def _tile_image(stem):
    key = f"TEX_bdtile_{stem}"
    img = bpy.data.images.get(key)
    if img is not None:
        return img
    p = BACKDROP_TILE_DIR / f"{stem}.png"
    if not p.exists():
        print(f"[mat_build] backdrop tile MISSING {p} -- run python3 scripts/env_p9_tiles.py")
        return None
    img = bpy.data.images.load(str(p), check_existing=True)
    img.name = key
    img.colorspace_settings.name = "Non-Color"      # it is a gain map, not an albedo
    return img
...
def backdrop_tile_detail(name, stem, strength, keep=0.55, uv_map="UVMap"):
    ...
    if m is None:
        ...
        return
    b = _principled_of(m)
    img = _tile_image(stem)
    if b is None or img is None:
        return
    t = Tree(m.node_tree)
    uv = t.new("ShaderNodeUVMap")
    uv.uv_map = uv_map
    tex = t.new("ShaderNodeTexImage", interpolation="Linear")
    tex.image = img
    ...
    if sk.is_linked:
        base = sk.links[0].from_socket
    else:
        base = t.rgb(tuple(sk.default_value)[:3])
    t.link(t.vmul(base, gain), sk)
    ML.finish(m)
