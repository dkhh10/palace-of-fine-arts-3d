...
def build_group_dome():
    ...
    sv = t.vadd(t.combxyz(t.mul(u, 1.3), t.mul(pz, 0.2), 0.0), t.vscale(off, 0.01))
    n1 = t.noise(sv, 1.0, detail=3, rough=0.6)
    n2 = t.noise(t.vscale(sv, 2.9), 1.0, detail=2, rough=0.5)
    smask = t.clamp01(t.madd(t.smoothstep(n2, 0.55, 0.75), 0.4, t.smoothstep(n1, 0.48, 0.7)))
    ...
    mp = t.smoothstep(t.noise(t.vadd(P, off), 0.7, detail=3, rough=0.6), 0.5, 0.68)
    moss = t.mul(t.mul(t.mul(north, mp), steep), I["Moss"])
    # grime ring at the base + gentle darkening of the lower flank
    ring_n = t.maprange(t.noise(t.vadd(P, off), 1.5, detail=2), 0.35, 0.65, -0.03, 0.03)
    grime = t.mul(t.maprange(nz, t.add(t.add(I["Base Normal Z"], 0.05), ring_n), t.add(I["Base Normal Z"], 0.01), 0.0, 1.0), I["Grime"])
    lower = t.mul(t.maprange(nz, 0.9, t.add(I["Base Normal Z"], 0.05), 0.0, 0.3), I["Grime"])
    # tone variation
...
def build_concrete_family():
    ...
    concrete_material("MAT_concrete_ochre", "concrete_wall_008", 1.0, {
        "Base Color": C(0.43, 0.285, 0.125), "Grey Color": C(0.30, 0.245, 0.165), "Grey Drift": 0.35,
        "Grey Below Z": 3.0, "Grey Above Z": 10.0, "Tone Variation": 0.15, "Block Size": 3.2, "Blotch Size": 1.8,
    ...
    concrete_material("MAT_concrete_podium", "concrete_wall_007", 2.0, {
        ...
        "Grey Below Z": 0.5, "Grey Above Z": 5.0, "Tone Variation": 0.11, "Block Size": 2.0, "Blotch Size": 2.5,
    ...
    concrete_material("MAT_concrete_colonnade", "concrete_wall_007", 3.0, {
        "Base Color": C(0.45, 0.30, 0.125), "Grey Color": C(0.28, 0.25, 0.18), "Grey Drift": 0.3,
        "Grey Below Z": 1.0, "Grey Above Z": 6.0, "Tone Variation": 0.12, "Block Size": 2.6, "Blotch Size": 3.0,
    ...
    concrete_material("MAT_concrete_inner", "concrete_wall_008", 4.0, {
        ...
        "Grey Below Z": 40.0, "Grey Above Z": 60.0, "Tone Variation": 0.11, "Block Size": 2.6, "Blotch Size": 2.5,
    ...
    concrete_material("MAT_ornament_concrete", "concrete_wall_008", 5.0, {
        "Base Color": C(0.43, 0.285, 0.125), "Grey Color": C(0.30, 0.245, 0.165), "Grey Drift": 0.25,
...
def build_dome():
    ...
    g = t.group(G["dome"], Normal=N, **{"Base Color": C(0.70, 0.66, 0.58), "Streak Color": C(0.50, 0.52, 0.52), "Moss Color": C(0.42, 0.47, 0.40),
                                        "Grime Color": C(0.20, 0.13, 0.06), "Seams": 48.0, "Seam Width": 0.05, "Streaks": 0.7, "Moss": 0.6,
...
def build_water():
    ...
    t.output(surface=bsdf.outputs[0], volume=addv.outputs[0])
    # Eevee: screen-space/raytraced refraction so the murk tint and reflections show up
